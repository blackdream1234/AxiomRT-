...
fn send_blocks_if_no_receiver() {
    ...
    assert_eq!(out, SendOutcome::Blocked, "send must block when nobody receives");
    assert_eq!(e.state(), EndpointState::SenderWaiting { sender: ThreadId(1) });
}
...
fn recv_blocks_if_no_sender() {
    ...
    assert_eq!(out, RecvOutcome::Blocked, "receive must block when nobody sends");
    assert_eq!(e.state(), EndpointState::ReceiverWaiting { receiver: ThreadId(2) });
}

#[test]
fn sender_first_rendezvous_delivers_and_unblocks() {
    let mut e = ep();
    assert_eq!(send(&mut e, ThreadId(1), msg(1, b"evt")).unwrap(), SendOutcome::Blocked);
    match recv(&mut e, ThreadId(2)).unwrap() {
        RecvOutcome::Received { msg, unblock } => {
            assert_eq!(msg.data(), b"evt");
            assert_eq!(msg.header().sender, ThreadId(1), "kernel-written sender identity");
...
fn bounded_no_second_sender_queue() {
    ...
    assert_eq!(send(&mut e, ThreadId(1), msg(1, b"a")), Err(IpcError::AlreadyWaiting));
}

#[test]
fn bounded_message_size_enforced_before_copy() {
    let too_big = [0u8; MSG_MAX_BYTES + 1];
    assert_eq!(Message::new(ThreadId(1), &too_big), Err(MessageError::TooLarge));
}
...
fn copy_semantics_no_shared_memory() {
    ...
    send(&mut e, ThreadId(1), Message::new(ThreadId(1), &source).unwrap()).unwrap();
...
fn deterministic_same_history_same_outcome() {
    for _ in 0..3 {
        let mut e = ep();
        assert_eq!(send(&mut e, ThreadId(1), msg(1, b"x")).unwrap(), SendOutcome::Blocked);
